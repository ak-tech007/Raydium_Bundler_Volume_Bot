  ...
  try {
    ...
    do {
      ...
      if (bundleStatus === "Landed") {
        ...
        return bundle?.result.value?.[0].transactions[0];
      }
...
export async function withdraw_pool(withdrawDetails: {
  percentage: number;
  mint: string;
  market_id: string;
  wallet: any;
}) {
  const { percentage, mint, market_id, wallet } = withdrawDetails;
  const provider = new anchor.AnchorProvider(
    connection,
    wallet,
    anchor.AnchorProvider.defaultOptions()
  ...
  const MARKET_KEY = new PublicKey(market_id); // Replace with OpenBook market key
  const COIN_MINT = new PublicKey(mint); // Replace with coin mint
  const PC_MINT = new PublicKey("So11111111111111111111111111111111111111112");
  ...
  const [ammAuthorityPda, ammAuthorityBump] = PublicKey.findProgramAddressSync(
    [Buffer.from("amm authority")],
    RAYDIUM_PROGRAM_ID
  );
  const [ammOpenOrdersPda, ammOpenOrdersbump] =
  ...
  const [ammCoinVaultPda, ammCoinVaultbump] = PublicKey.findProgramAddressSync(
    [
      RAYDIUM_PROGRAM_ID.toBuffer(),
      MARKET_KEY.toBuffer(),
      Buffer.from("coin_vault_associated_seed"),
  ...
  const MARKET_COIN_VAULT_KEY = new PublicKey(market_keys.marketCoinVault);
  const MARKET_PC_VAULT_KEY = new PublicKey(market_keys.marketPcVault);
  const MARKET_VAULT_SIGNER_KEY = new PublicKey(market_keys.marketVaultSigner);

  const LP_balance = await getTokenBalance(wallet.publicKey, ammLpMintPda);

  try {
    const transaction_withdraw = new Transaction();
    transaction_withdraw.add(
      await program.methods
        .withdraw(
          new anchor.BN(BigInt(Math.floor((LP_balance * percentage) / 100))),
          new anchor.BN(BigInt(0)),
          new anchor.BN(BigInt(0))
        )
        .accounts({
          ammProgram: RAYDIUM_PROGRAM_ID,
          ...
          marketBids: MARKET_BIDS_KEY,
          marketAsks: MARKET_ASKS_KEY,
          userLpTokenAccount: user_token_lp,
          userCoinAccount: user_token_coin,
          userPcAccount: user_token_pc,
          userWallet: wallet.publicKey,
          splToken: tokenProgram,
        })
        .instruction()
    ...
    transaction_withdraw.recentBlockhash = (
      await connection.getLatestBlockhash()
    ).blockhash;
    transaction_withdraw.feePayer = wallet.publicKey;

    const signature_withdraw =
      await wallet.signTransaction(transaction_withdraw);

    const signature = await connection.sendRawTransaction(
      signature_withdraw.serialize()
    );

    console.log("Transaction sent, awaiting confirmation...");
    await connection.confirmTransaction(signature);
    console.log("Transaction confirmed:", signature_withdraw);
  } catch (error) {
